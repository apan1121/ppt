				...
				if( i < index ) {
					...
					if( config.fragments ) {
						...
						while( pastFragments.length ) {
							...
							var tmpfragmentIndex = parseInt( pastFragment.getAttribute("data-fragment-index"),10 );
							if(typeof(tmpfragmentIndex)=="number"){
								if(tmpfragmentIndex > fragmentIndex){
									fragmentIndex = tmpfragmentIndex;
								}
							}
						}
						...
						while( pastFragments.length ) {
							...
							var tmpfragmentIndex_out = pastFragment.getAttribute("data-fragment-index-out");
							if( tmpfragmentIndex_out!=  null ){
								if( tmpfragmentIndex <fragmentIndex  ){
									pastFragment.classList.remove( 'visible' );
								}
							}
						}
					}
	...
	function sortFragments( fragments ) {
		...
		ordered.forEach( function( group ) {
			group.forEach( function( fragment ) {
				...
				var org_index = fragment.getAttribute( 'data-fragment-index');
				var o = org_index - index;

				fragment.setAttribute( 'data-fragment-index', index );
				if(	 fragment.hasAttribute( 'data-fragment-index-out') ){
					var org_index_out = parseInt(fragment.getAttribute( 'data-fragment-index-out'));
					if(typeof( org_index_out ) =="number"){
						org_index_out = parseInt( org_index_out,10 ) - o

						fragment.setAttribute( 'data-fragment-index-out', org_index_out );
					}
				}
		...
		if( currentSlide && config.fragments ) {
			...
			if( fragments.length ) {
				...
				toArray( fragments ).forEach( function( element, i ) {
					...
					var o = index + 1 ;
					if( element.hasAttribute( 'data-fragment-index-out' ) ) {
						o = parseInt( element.getAttribute( 'data-fragment-index-out' ), 10 );
					}
